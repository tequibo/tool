...
class LoopScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        physicsBody = SKPhysicsBody(
            rectangleOf: CGSize(width: view.bounds.width, height: view.bounds.height),
            center: CGPoint(x: view.bounds.width / 2, y: view.bounds.height / 2))
    ...
    private func addImageSequence(fromFolder folderURL: URL, at location: CGPoint) {
        ...
        do {
            ...
            let wavFileExists =
                fileManager.fileExists(atPath: wavFileURL.path, isDirectory: &isDirectory)
            ...
            let allFileUrlsInFolder = try fileManager.contentsOfDirectory(
            ...
            addImageSequence(
                fromFiles: imageUrls, at: location, settings: self.viewModel, audioURL: audioURL,
                addToNumbered: true)
    ...
    private func addImageSequence(
        fromFiles urls: [URL], at location: CGPoint, settings: SharedObject, audioURL: URL? = nil,
        addToNumbered: Bool = false
    ) {
        ...
        let magicLoop = MagicLoop(
            textures: textures, position: location, settings: viewModel, audioURL: audioURL)
    ...
    func loadSequencesFromPresetFolder() {
        ...
        do {
            ...
            for folderURL in folderContents where folderURL.hasDirectoryPath {
                let yPos = -i * clipHeight + self.size.height * 2 - 128
                let xPos = 100.0
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        wigglePosition = CGPoint(
            x: brush.x + cos(currentTime * wiggleFrequency.dx) * frame.width / 2 * wiggleScale.dx,
            y: brush.y + sin(currentTime * wiggleFrequency.dy) * frame.height / 2 * wiggleScale.dy)
        ...
        selectedBrush?.maskCenter.vectorFloat2Value = vector_float2(
            Float(maskPosition.x), Float(maskPosition.y))
        selectedBrush?.zoom.vectorFloat2Value = vector_float2(
            Float(brushZoom.x), Float(brushZoom.y))
        if isPainting {
            counter += 1
        ...
        if isRecordingPosition {
            if let selectedLoop = selectedNode as? MagicLoop {
                ...
                //				print(targetFrame)
                selectedLoop.positionRecord[targetFrame].x =
                    mousePosition.x - selectedLoop.truePosition.x
                selectedLoop.positionRecord[targetFrame].y =
                    mousePosition.y - selectedLoop.truePosition.y
            }
        }
        if isRecordingScale {
            if let selectedLoop = selectedNode as? MagicLoop {
                ...
                let targetFrame = (selectedLoop.currentFrame + 1) % selectedLoop.totalFrames
                let targetXScale =
                    -(selectedLoop.truePosition.x - mousePosition.x)
                    / selectedLoop.spriteNode.frame.width * 2
                let targetYScale =
                    (selectedLoop.truePosition.y - mousePosition.y)
                    / selectedLoop.spriteNode.frame.height * 2
        ...
        if record, let selectedDirectory = selectedDirectory {
            ...
            if currentFrameExport > framesToExport - 1 {
                saveFramesAsMovie(
                    to: selectedDirectory, prompt: prompt, dateTimeString: dateTimeString,
                    frames: frames, frameRate: Int(self.viewModel.fpsExport))
                toggleExport()
    ...
    private func saveFrameAsImage(to directoryURL: URL, prompt: String, dateTimeString: String) {
        ...
        do {
            try FileManager.default.createDirectory(
                at: sequenceFolder, withIntermediateDirectories: true, attributes: nil)
        } catch {
        ...
        let scaledExportFrame = exportFrame.applying(
        ...
        let side = 1024 * 2
        let myFrame: CGRect = CGRect(
            x: Int(fullCGImage.width) / 2 - side / 2, y: Int(fullCGImage.height) / 2 - side / 2,
        ...
        guard
            let resizedImage = resizeImage(
                image: croppedCGImage, to: CGSize(width: 512, height: 512))
        else { return }
    ...
    //MARK: SAVE MOVIE
    private func saveFramesAsMovie(
        to directoryURL: URL, prompt: String, dateTimeString: String, frames: [CGImage],
        frameRate: Int
    ) {
        ...
        let adaptor = AVAssetWriterInputPixelBufferAdaptor(
            assetWriterInput: writerInput, sourcePixelBufferAttributes: sourceBufferAttributes)
        ...
        writer.finishWriting {
            if writer.status == .completed {
                ...
                print(
                    "Failed to save movie: \(writer.error?.localizedDescription ?? "unknown error")"
                )
            }
        }
    }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        case Keycode.i:
            ...
            let colorize = SKAction.colorize(
                with: targetColor, colorBlendFactor: 1.0, duration: 0.2)
            self.run(colorize)
        ...
        case Keycode.c:
            if editorMode == Modes.writeChange {
                editorMode = Modes.drag
                transformCurrent.position = CGPoint(
                    x: selectedBrush!.truePosition.x - transformOriginal.position.x,
                    y: selectedBrush!.truePosition.y - transformOriginal.position.y)
                transformCurrent.scale = CGPoint(
                    x: selectedBrush!.trueScale.x - transformOriginal.scale.x,
                    y: selectedBrush!.trueScale.y - transformOriginal.scale.y)
            } else if editorMode == Modes.drag {
                transformOriginal.position = selectedBrush!.truePosition
                transformOriginal.scale = selectedBrush!.trueScale
        ...
        case Keycode.m:
            ...
            let scaledDistance =
                distance / (selectedLoop.spriteNode.frame.width * selectedLoop.trueScale.x)
        ...
        case Keycode.m:
            ...
            let scaledDistance =
                distance / (selectedLoop.spriteNode.frame.width * selectedLoop.trueScale.x)
            selectedLoop.maskCenter.vectorFloat2Value = vector_float2(
                Float(0.5 - deltaX / divider), Float(0.5 - deltaY / divider))
        ...
        case Keycode.minus:
            ...
            selectedLoop.targetDelta =
                loopTime / Double(selectedLoop.textures.count) * selectedLoop.loopTimeMultiplier
    ...
    func duplicateLastDraggedNode() {
        ...
        if let spriteNode = lastNode as? SKSpriteNode, let texture = spriteNode.texture {
            ...
            newNode.position = CGPoint(
                x: selectedBrush?.position.x ?? 0, y: selectedBrush?.position.y ?? 0)
            ...
            newImageNode.truePosition.x += transformCurrent.position.x
            newImageNode.truePosition.y += transformCurrent.position.y
            newImageNode.trueScale.x += transformCurrent.scale.x
            newImageNode.trueScale.y += transformCurrent.scale.y
    ...
    func createEchoes() {
        ...
        if let lastLoop = lastNode as? MagicLoop {
            for i in 0...steps {
                ...
                let newLoop = lastLoop.duplicate(
                    at: CGPoint(
                        x: (selectedBrush?.truePosition.x)!, y: (selectedBrush?.truePosition.y)!))
                ...
                newLoop.trueScale = CGPoint(
                    x: lastLoop.trueScale.x + transformCurrent.scale.x * CGFloat(i),
                    y: lastLoop.trueScale.y + transformCurrent.scale.y * CGFloat(i))
                newLoop.truePosition.x =
                    newLoop.truePosition.x + (transformCurrent.position.x) * CGFloat(i)
                newLoop.truePosition.y =
                    newLoop.truePosition.y + (transformCurrent.position.y) * CGFloat(i)
                newLoop.threshold.floatValue = Float(
                    map(value: fractC, fromLow: 0, fromHigh: 1, toLow: 0.0, toHigh: 1.0))
    ...
    func _createEchoes() {
        ...
        if let lastLoop = lastNode as? MagicLoop {
            for i in 0...steps {
                ...
                let newLoop = lastLoop.duplicate(
                    at: CGPoint(x: self.frame.width / 2, y: self.frame.height / 2))
                //                newLoop.trueRotation = fractionOfCircumference*CGFloat.tau
                for j in 0...(newLoop.textures.count - 1) {
                    let a2 = Double(j) / Double(newLoop.textures.count - 1)
                    let s = Double(sin(a2 * Double.tau) * 0.1)
                ...
                newLoop.trueScale = CGPoint(
                    x: lastLoop.trueScale.x + addedScale, y: lastLoop.trueScale.y + addedScale)
        ...
        if let lastLoop = lastNode as? MagicLoop {
            ...
            for h in 1...rings {
                ...
                for i in 0...steps_ {
                    ...
                    newLoop.truePosition = CGPoint(
                        x: self.frame.width / 2 + cos(a * Double.tau) * dist,
                        y: self.frame.height / 2 + sin(a * Double.tau) * dist)
                    ...
                    newLoop.timeOffset =
                        Double(lastLoop.loopDuration) * a + (Double(lastLoop.loopDuration)) * a0
        ...
        if let lastLoop = lastNode as? MagicLoop {
            for j in 0...side {
                ...
                for i in 0...side {
                    ...
                    //rectangles
                    newLoop.params.vectorFloat4Value = vector_float4(
                        Float(side), Float(side), Float(j), Float(i))
                    //vertical slices
                    //					newLoop.params.vectorFloat4Value = vector_float4(Float(side*side), 1, Float(n), 0)

                    //horizontal slices
                    newLoop.params.vectorFloat4Value = vector_float4(
                        1, Float(side * side), 0, Float(n))
        ...
        if let lastLoop = lastNode as? MagicLoop {
            for h in 0...steps {
                ...
                for i in 0...steps {
                    ...
                    newLoop.currentFrame =
                        lastLoop.currentFrame + newLoop.textures.count * i / steps
                    print("-----")
                    print(i)
                    print(a)
                    print("steps: \(steps)")
    ...
    func processMagicLoopWithControlNet(_ magicLoop: MagicLoop, _ test: Bool = false) {
        ...
        var frames = magicLoop.textures.map { texture in
            ...
            let nsImage = NSImage(
                cgImage: cgImage(), size: NSSize(width: size.width, height: size.height))
            return nsImage
        }
        if test {
        ...
        print(self.prompt)
        sendFramesToServer(frames: frames, prompt: self.prompt, steps: self.steps) {
            generatedFrames in
    ...
    func sendFramesToServer(
        frames: [NSImage], prompt: String, steps: Int, completion: @escaping ([NSImage]) -> Void
    ) {
        ...
        let body = createMultipartBody(
            with: frames, boundary: boundary, prompt: prompt, steps: steps)
    ...
    func createMultipartBody(with frames: [NSImage], boundary: String, prompt: String, steps: Int)
        -> Data
    {
        print(prompt)
        print(steps)
    ...
    func createNewMagicLoop(with frames: [NSImage]) {
        ...
        let newMagicLoop = MagicLoop(
            textures: textures, position: CGPoint(x: self.size.width / 2, y: self.size.height / 2),
            settings: self.viewModel)
